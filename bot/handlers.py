from aiogram import Router, F
from aiogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery
from telethon_client.parser import query_external_bot, download_audio
from bot.config import DOWNLOAD_PATH


router = Router()
user_cache = {}


@router.message(F.text)
async def handle_search(message: Message):
options = await query_external_bot(message.text)


if not options:
await message.answer("❌ No audio found")
return


keyboard = []
for idx, opt in enumerate(options):
user_cache[f"{message.from_user.id}:{idx}"] = opt["file_id"]
keyboard.append([
InlineKeyboardButton(
text=f"🎵 {opt['performer']} - {opt['title']}",
callback_data=str(idx)
)
])


await message.answer(
"🎧 Select audio:",
reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard)
)


@router.callback_query()
async def handle_download(call: CallbackQuery):
await call.answer()


key = f"{call.from_user.id}:{call.data}"
msg_id = user_cache.get(key)


if not msg_id:
await call.message.edit_text("❌ Session expired")
return


filename = await download_audio(msg_id, DOWNLOAD_PATH)


if filename:
await call.message.edit_text(f"✅ Downloaded: {filename}")
else:
await call.message.edit_text("❌ Download failed")
```python
from aiogram import Router
from aiogram.types import Message, InlineKeyboardMarkup, InlineKeyboardButton, CallbackQuery
from telethon_client.parser import query_external_bot, download_latest_file
from bot.config import DOWNLOAD_PATH


router = Router()


@router.message()
async def handle_query(message: Message):
responses = await query_external_bot(message.text)


buttons = []
for i, r in enumerate(responses):
buttons.append([
InlineKeyboardButton(text=f"Option {i+1}", callback_data=str(i))
])


await message.answer(
"Choose an option:",
reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons)
)


@router.callback_query()
async def handle_choice(call: CallbackQuery):
await call.answer()
index = call.data


success = await download_latest_file(
filename=f"selection_{index}.bin",
path=DOWNLOAD_PATH
)


if success:
await call.message.edit_text("✅ Download completed")
else:
await call.message.edit_text("❌ No file received")
